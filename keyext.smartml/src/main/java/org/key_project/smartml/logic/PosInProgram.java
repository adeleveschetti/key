...
public final class PosInProgram {
    ...
    /**
     * returns the SmartMLProgramElement at the given position
     *
     * @param pos the PosInProgram
     * @param prg the SmartMLProgramElement we walk through
     * @return the SmartMLProgramElement at the given position
     * @throws IndexOutOfBoundsException if position <code>pos</code> refers to a non-existent
     *         program element
     */
    public static SmartMLProgramElement getProgramAt(PosInProgram pos, SmartMLProgramElement prg) {
        SmartMLProgramElement result = prg;
        for (int i = 0; i < pos.depth; i++) {
            // method getChildAt throws an array index out of bound if
            // it.next refers to a non-existing child
            result = (SmartMLProgramElement) result.getChild(pos.pos[i]);
        }
    ...
    public SmartMLProgramElement getProgram(SmartMLProgramElement pe) {
