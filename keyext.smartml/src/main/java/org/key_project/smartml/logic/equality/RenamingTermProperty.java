...
import static org.key_project.smartml.logic.equality.RenamingProgramElementProperty.RENAMING_PROGRAM_ELEMENT_PROPERTY;
...
public class RenamingTermProperty implements Property<Term> {
     ...
     *         varsBoundHere and SmartMLBlock as {@code term1} modulo bound renaming
    ...
    private boolean unifyHelp(Term t0, Term t1, ImmutableList<QuantifiableVariable> ownBoundVars,
        ...
        if (op0 instanceof Modality mod0 && op1 instanceof Modality mod1) {
            ...
            nat = handleSmartML(mod0.program(), mod1.program(), nat);
            if (nat == FAILED) {
                return false;
            }
    ...
    /**
     * used to encode that <tt>handleSmartML</tt> results in an unsatisfiable constraint (faster than
     * using exceptions)
     */
    private static final NameAbstractionTable FAILED = new NameAbstractionTable();

    /**
     * Checks whether the given {@link SmartMLBlock}s are equal modulo renaming and returns the
     * updated
     * {@link NameAbstractionTable} or {@link #FAILED} if the {@link SmartMLBlock}s are not equal.
     *
     * @param jb0 the first {@link SmartMLBlock} to compare
     * @param jb1 the second {@link SmartMLBlock} to compare
     * @param nat the {@link NameAbstractionTable} used for the comparison
     * @return the updated {@link NameAbstractionTable} if the {@link SmartMLBlock}s are equal modulo
     *         renaming or {@link #FAILED} if they are not
     */
    private static NameAbstractionTable handleSmartML(SmartMLBlock jb0, SmartMLBlock jb1,
            NameAbstractionTable nat) {
        if (!jb0.isEmpty() || !jb1.isEmpty()) {
            nat = checkNat(nat);
            if (SmartMLBlocksNotEqualModRenaming(jb0, jb1, nat)) {
                return FAILED;
            }
        }
        return nat;
    ...
    /**
     * Returns true if the given {@link SmartMLBlock}s are not equal modulo renaming.
     *
     * @param rb1 the first {@link SmartMLBlock}
     * @param rb2 the second {@link SmartMLBlock}
     * @param nat the {@link NameAbstractionTable} used for the comparison
     * @return true if the given {@link SmartMLBlock}s are NOT equal modulo renaming
     */
    public static boolean SmartMLBlocksNotEqualModRenaming(SmartMLBlock rb1, SmartMLBlock rb2,
            NameAbstractionTable nat) {
        SmartMLProgramElement pe1 = rb1.program();
        SmartMLProgramElement pe2 = rb2.program();
     ...
     * @return the hash code
     */
    private int hashSmartMLBlock(Modality mod) {
        final SmartMLBlock rb = mod.program();
        if (!rb.isEmpty()) {
