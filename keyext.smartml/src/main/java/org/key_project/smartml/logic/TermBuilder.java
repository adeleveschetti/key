...
public class TermBuilder {
    ...
    public Term equals(Term t1, Term t2) {
        if (t1.sort() == SmartMLDLTheory.FORMULA) {
    ...
    public Term prog(Modality.SmartMLModalityKind modKind, SmartMLBlock jb, Term t) {
        return tf.createTerm(Modality.getModality(modKind, jb), new Term[] { t }, null);
    }

    public Term box(SmartMLBlock jb, Term t) {
        return prog(Modality.SmartMLModalityKind.BOX, jb, t);
    }

    public Term dia(SmartMLBlock jb, Term t) {
        return prog(Modality.SmartMLModalityKind.DIA, jb, t);
    }
    ...
    public Term parallel(Term u1, Term u2) {
        if (u1.sort() != SmartMLDLTheory.UPDATE) {
            throw new TermCreationException("Not an update: " + u1);
        } else if (u2.sort() != SmartMLDLTheory.UPDATE) {
    ...
    public Term apply(Term update, Term target) {
        if (update.sort() != SmartMLDLTheory.UPDATE) {
