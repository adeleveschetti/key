...
package org.key_project.smartml.rule.match;
...
import org.key_project.smartml.logic.op.UpdateApplication;
...
public class VMTacletMatcher implements TacletMatcher {
    ...
    private final org.key_project.smartml.rule.match.TacletMatchProgram findMatchProgram;
    /** the matcher for the taclet's assumes formulas */
    private final HashMap<Term, org.key_project.smartml.rule.match.TacletMatchProgram> assumesMatchPrograms = new HashMap<>();
    ...
    public VMTacletMatcher(Taclet taclet) {
        ...
        if (taclet instanceof FindTaclet ft) {
            ...
            findMatchProgram = org.key_project.smartml.rule.match.TacletMatchProgram.createProgram(findExp);

        } else {
            ignoreTopLevelUpdates = false;
            findExp = null;
            findMatchProgram = org.key_project.smartml.rule.match.TacletMatchProgram.EMPTY_PROGRAM;
        }

        for (var sf : assumesSequent) {
            assumesMatchPrograms.put(sf.formula(), org.key_project.smartml.rule.match.TacletMatchProgram.createProgram(sf.formula()));
        }
    }
    ...
    public org.key_project.prover.rules.instantiation.MatchConditions matchFind(
        ...
        if (findMatchProgram == org.key_project.smartml.rule.match.TacletMatchProgram.EMPTY_PROGRAM) {
            return null;
        }
    ...
    public final AssumesMatchResult matchAssumes(Iterable<AssumesFormulaInstantiation> toMatch,
        ...
        org.key_project.smartml.rule.match.TacletMatchProgram prg = assumesMatchPrograms.get(p_template);
    ...
    public MatchConditions matchSV(SchemaVariable sv,
        ...
        } else if (syntaxElement instanceof SmartMLProgramElement pe) {
