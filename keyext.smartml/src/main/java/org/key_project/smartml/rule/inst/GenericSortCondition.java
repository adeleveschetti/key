...
import org.key_project.smartml.logic.op.sv.TermSV;
import org.key_project.smartml.logic.sort.GenericSort;
import org.key_project.smartml.rule.inst.GenericSortInstantiations;
import org.key_project.smartml.rule.inst.TermInstantiation;
...
public abstract class GenericSortCondition {
    ...
    protected static GenericSortCondition createCondition(Sort s0, Sort s1, boolean p_identity) {
        if (!(s0 instanceof GenericSort gs) || s1 == SmartMLDLTheory.FORMULA
                || s1 == SmartMLDLTheory.UPDATE) {
            return null;
        }
